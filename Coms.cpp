...
#define handShake "HND|-1|CodyWebb"
...
int WifiComs::WifiConnection(char ReplyBuffer[]){
    ...
    if(wifiState == 1 && WiFi.status() == WL_CONNECTED){
      ...
      Udp.write("State is 2, sending AmDrone to basestation"); 
      Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
      Udp.write("AmDrone");
      Udp.endPacket();
      ...
      Serial.println("State is 4, handshake with basestation");
      Udp.beginPacket(bsip, 5005);
      Udp.write(handShake);
      Udp.endPacket();
      Serial.println(handShake);
      Serial.println("-- Handshake message");
      Serial.println(bsip);
      Serial.println("State is 4 -> 5");
      wifiState = 5;
      return 5;
    }
    else
