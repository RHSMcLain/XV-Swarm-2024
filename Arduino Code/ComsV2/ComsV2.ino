...
void loop(){
	wifi.WifiConnection(ReplyBuffer);                       //Reconnect WiFi
	wifi.Listen(packetBuffer);                              //Listen for new messages
	...
	if(wifi.PrevMessage.cmd == "MAN"){
		...
		if(wifi.state == "active"){
			...
			Serial.println("ERROR - State not recognized: " + wifi.PrevMessage.state);
